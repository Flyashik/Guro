    ...
    public partial class Form1 : Form
    {
        ...
        public void Z_buffer()
        {
            ...
            foreach (var edge in temp_edges)
            {
                ...
                normal4 = new Point3(normal4.X + centr.X, normal4.Y + centr.Y, normal3.Z + centr.Z);
                        ...
                        
                        {
                            if (InTriangle(p1, p2, p3, new Point3(x, y, 0)))
                            {
                                ...
                                if (z > buffer[x, y])
                                {
                                    ...
                                    //int left_x = x;
                                    //int right_x = x;
                                    Point3 left_point = new Point3(x - 1, y, 0);
                                    Point3 right_point = new Point3(x + 1, y, 0);
                                    while (!PointOnEdge(p1, p2, p3, left_point))
                                    {
                                        if (left_point.X - 1 > 0)
                                            left_point.X -= 1;
                                        else
                                    ...
                                    while (!PointOnEdge(p1, p2, p3, right_point))
                                    {
                                        if (right_point.X + 1 < pictureBox1.Width)
                                            right_point.X += 1;
                                        else
                                            break;
                                    }
                                    //if (!(left_point.X - 1 <= 0 || right_point.X + 1 >= pictureBox1.Width))
                                    {
                                        double ia = intensive1 * ((y - p2.Y) / (p1.Y - p2.Y)) + intensive2 * ((p1.Y - y) / (p1.Y - p2.Y));
                                        double ib = intensive1 * ((y - p3.Y) / (p1.Y - p3.Y)) + intensive3 * ((p1.Y - y) / (p1.Y - p3.Y));
                                        double ip = ia * ((right_point.X - x) / (right_point.X - left_point.X)) + ib * ((x - left_point.X) / (right_point.X - left_point.X));

                                        double ip2 = ia * ((left_point.X - x) / (left_point.X - right_point.X)) + ib * ((x - right_point.X) / (left_point.X - right_point.X));
                        ...
                        if (edge.points.Count == 4)
                        {
                            if (InTriangle(p1, p3, p4, new Point3(x, y, 0)))
                            {
                                ...
                                if (z > buffer[x, y])
                                {
                                    ...
                                    //int left_x = x;
                                    //int right_x = x;
                                    //Здесь должна была быть интерполяция
                                    Point3 left_point = new Point3(x - 1, y, 0);
                                    Point3 right_point = new Point3(x + 1, y, 0);
                                    while (!PointOnEdge(p1, p3, p4, left_point))
                                    {
                                        if (left_point.X - 1 > 0)
                                            left_point.X -= 1;
                                        else
                                    ...
                                    while (!PointOnEdge(p1, p3, p4, right_point))
                                    {
                                        if (right_point.X + 1 < pictureBox1.Width)
                                            right_point.X += 1;
                                        else
                                    ...
                                    //if (!(left_point.X - 1 <= 0 || right_point.X + 1 >= pictureBox1.Width))
                                    {
                                        double ia = intensive1 * ((y - p3.Y) / (p1.Y - p3.Y)) + intensive3 * ((p1.Y - y) / (p1.Y - p3.Y));
                                        double ib = intensive1 * ((y - p4.Y) / (p1.Y - p4.Y)) + intensive4 * ((p1.Y - y) / (p1.Y - p4.Y));
                                        double ip = ia * ((right_point.X - x) / (right_point.X - left_point.X)) + ib * ((x - left_point.X) / (right_point.X - left_point.X));
